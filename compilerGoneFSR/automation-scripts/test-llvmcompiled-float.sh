...
file=$testDir/solution-llvmcompile-float
echo "floattest" >> $file.txt
echo "Solution >>>" >> $file.txt
python3 -m goner.compile Tests/floattest.g 2> /dev/null
./a.out >>  $file.txt	2> /dev/null
echo "<<<<" >> $file.txt
echo "My implementation solution" >> $file.txt
python3 -m gone.compile Tests/floattest.g 2> /dev/null
